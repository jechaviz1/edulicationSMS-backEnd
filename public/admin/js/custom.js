	...
	var pageOnScroll = function(event){
		...
		if(jQuery('.navbar-nav').length > 0){
			...
			jQuery(document).on("scroll", function(){
				...
				jQuery('.demo-view .dz-card').each(function () {
					var target = $(this);
					var targetTop = target.offset().top;
					var targetHeight = target.height();
					var targetId = target.attr('id');
					var navLink = $('a[href="#' + targetId + '"]');
...
function getCountUnits(releases) {
	return releases.reduce((result, item) => {
		if (item?.Currency === 'Current') {
			const totalUnits = item.UnitGrid.UnitGridEntry.length;
			const essentialUnits = item.UnitGrid.UnitGridEntry.filter(unit => unit?.IsEssential).length;
			result.coreUnits += essentialUnits;
			result.totalUnits += totalUnits;
			result.curUnits = item.UnitGrid.UnitGridEntry;
		}
		return result;
	}, { coreUnits: 0, totalUnits: 0, curUnits: [] });
}