...
class USIService extends SoapHelper
{
    ...
    const ACTION_ISSUE_BULK_VERIFY = 'http://usi.gov.au/2022/ws/BulkVerifyUSI';
    ...
    const VANURL_TEST = 'https://softwareauthorisations.evte.ato.gov.au/R3.0/S007v1.3/service.svc';
    ...
    public function verifyBulkUSI($data)
    {
        $this->isBulk = true;
        $this->bulkData = $data;
        ...
        try {
            $response = $this->sendSoapRequest(self::ACTION_ISSUE_BULK_VERIFY, $xmlRequest, $this->usiUrl);
        ...
        if(config('usi.mode') === 'TEST') $this->debug('verify_response.xml', $rawResponse);

        $response = $xml->xpath("//ws:BulkVerifyUSIResponse");
        if (count($response)>0) {
            $response = $response[0];
            //dd($response->VerificationResponses);

            /* Convert result into an array instead of simpleXMLElement */
            $output = array();
            foreach ($response->VerificationResponses->VerificationResponse as $name => $value) {
                $output[] = $value;
            }
            return $output;
        } else {
    ...
    public function verifyUSI($usi, $first, $family, $dob)
    {
    ...
    private function buildRSTdocument()
    {
        ...
        if(config('usi.mode') === 'TEST') $this->debug('request.xml');
    ...
    private function buildVerifyRequest($rawMASResp)
    {
        ...
        $xml = $this->doc->saveXML();
        if(config('usi.mode') === 'TEST') $this->debug('request-usi.xml');
    ...
    private function buildSoapHeader($rawMASResp = null)
    {
        ...
        if ($rawMASResp) {
            if($this->isBulk)
                $action = $this->doc->createElementNS(self::NS_ADDR, self::NS_ADDR_PREFIX . ':Action', self::ACTION_ISSUE_BULK_VERIFY);
            else
    ...
    private function buildSoapBodyUSI()
    {
        ...
        if($this->isBulk)
        {
            $bulkVerifyUSI = $this->doc->createElementNS(self::NS_USI, self::NS_USI_PREFIX . ':BulkVerifyUSI');
            $bulkVerifyUSI->appendChild($this->doc->createElementNS(self::NS_USI, self::NS_USI_PREFIX . ':OrgCode', $this->orgCode));
            $bulkVerifyUSI->appendChild($this->doc->createElementNS(self::NS_USI, self::NS_USI_PREFIX . ':NoOfVerifications', count($this->bulkData)));
            $verifications = $this->doc->createElementNS(self::NS_USI, self::NS_USI_PREFIX . ':Verifications');
            foreach ($this->bulkData as $key => $data) {
                $verification = $this->doc->createElementNS(self::NS_USI, self::NS_USI_PREFIX . ':Verification');
                $verification->appendChild($this->doc->createElementNS(self::NS_USI, self::NS_USI_PREFIX . ':RecordId', $data['record_id']));
                $verification->appendChild($this->doc->createElementNS(self::NS_USI, self::NS_USI_PREFIX . ':USI', $data['usi']));

                // Handle first and family name
                if (empty($data['family_name'])) {
                    $verification->appendChild($this->doc->createElementNS(self::NS_USI, self::NS_USI_PREFIX . ':SingleName', $data['first_name']));
                } elseif (empty($data['first_name'])) {
                    $verification->appendChild($this->doc->createElementNS(self::NS_USI, self::NS_USI_PREFIX . ':SingleName', $data['family_name']));
                } else {
                    $verification->appendChild($this->doc->createElementNS(self::NS_USI, self::NS_USI_PREFIX . ':FirstName', $data['first_name']));
                    $verification->appendChild($this->doc->createElementNS(self::NS_USI, self::NS_USI_PREFIX . ':FamilyName', $data['family_name']));
                }
                $verification->appendChild($this->doc->createElementNS(self::NS_USI, self::NS_USI_PREFIX . ':DateOfBirth', $data['date_of_birth']));

                // Append the verification to the verifications
                $verifications->appendChild($verification);
            }
            $bulkVerifyUSI->appendChild($verifications);
            $body->appendChild($bulkVerifyUSI);
