...
class StaffHourlyAttendanceController extends Controller {
    ...
    public function index(Request $request) {
        $data = [];
        $data['title'] = 'Staff Hourly Attendance';
        $data['menu_active_tab'] = 'staff_hourly_attendance';
        ...
        $data['users'] = Employee::where('salary_type', '2')->orderBy('id', 'asc')->get();


        // Filter Users
        if(!empty($request->staff)){

            $user = Employee::where('id', $staff)->where('salary_type', '2')->where('status', '1')->first();
        ...
        if(!empty($request->staff) && !empty($request->date)){
            ...
            $data['attendances'] = StaffHourlyAttendance::where('employ_id', $request->staff)
                        ->where('date', $date)
                        ->get();
        }
        // dd($data);
        
        return view('admin.staff_hourly_attendance.index')->with($data);
    }
